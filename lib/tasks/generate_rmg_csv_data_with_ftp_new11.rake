...
task generate_rmg_csv_data_with_ftp_new11: :environment do
  require 'csv'
  require 'tempfile'
  require 'net/sftp'
  ...
  header = ['Phone Number', 'Timestamp ET', 'Message', 'Email'].freeze

  Message.select(:id, :to, :created_at, :text, :user_id)
         .where('user_id in (?)', ids) # .where('created_at > ?', '2018-01-01 03:59:59') # .where("created_at < ?", "2019-09-01 04:00:00")
    ...
    if messages.present?
      ...
      csv_string = CSV.generate do |csv|
        ...
        messages.each do |m|
          ...
          # end
          csv << [m.to, m.created_at.to_time.strftime('%Y-%m-%d %H:%M:%S'), m.text, users[m.user_id]]
        end
      end

...
=begin
 ...
 # some cleanups
  ...
  # rhombus number don't exist...causes orphaned messages since we pull by ids
  # this is an attempt to fix it but it might not catch all
  select * from messages where user_id_from = 0 # user_id_to = 0
  select * from users where phone_number in (select rhombus_number from users)
  select * from messages where user_id_from = user_id_to 
...
task :update_customer_number_data => :environment do
  ...
  ActiveRecord::Base.transaction do  
    Message.find_each do |m|  
      ...
      # update each message using user_ids to most current user details if user_id exists
      ...
      if u = User.find_by(id: m.user_id) 
        ...
        elsif u.is_merchant? && u.rhombus_number.present?
      ...
      puts m.changes.inspect if m.changed?
      m.save!
      puts "moving on... \n"
    end
  end

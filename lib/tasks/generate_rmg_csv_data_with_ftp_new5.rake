...
task generate_rmg_csv_data_with_ftp_new5: :environment do
  ...
  users.each_with_index do |user, i|
    ...
    Message.select(:id, :from, :to, :created_at, :text, :user_id, :user_id_to).where('user_id = ? OR user_id_to = ?',
                                                                                     user.id, user.id).find_in_batches(batch_size: 999_998).with_index do |messages, index|
      if messages.present?
        puts "batch #{index + 1}"
        recipient = nil
        direction = nil

...
task generate_rmg_csv_data_with_ftp_new2: :environment do
  ...
  users = User.where(email: ['<redacted_email>', '<redacted_email>', '<redacted_email>'])
...
=end
  ...
  remote_folder = "/DataGoesHere/Dec 16, 2019 Request 1" #{date} Funnel Campaigns - All Accounts"
  ...
  users.each_with_index do |user,i|
    ...
    if true #first_campaign.try(:created_at).present?
      ...
      #messages = Message.find_by_sql([query_string, user.id, user.id, '2017-09-03 04:00:00'])#, '2019-07-01 04:00:00'])
      messages = Message.find_by_sql([query_string, user.id, user.id])

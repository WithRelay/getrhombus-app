...
task generate_rmg_csv_data_with_ftp_new2: :environment do
  ...
  users = User.where(id: [48253, 48254, 48255, 48256])
  ...
  #since_date_time = (Time.now.utc - 24.hours).to_s(:db).freeze #'2019-06-21 07:00:00'
  since_date_time = (Time.now.utc - 40.hours).to_s(:db).freeze
  max_date_time = (Time.now.utc - 24.hours).to_s(:db).freeze
  date = (DateTime.now - 24.hours).strftime("%b %d, %Y").freeze #(DateTime.now).strftime("%b %d, %Y").freeze
  remote_folder = "/DataGoesHere/#{date} Funnel Campaigns - All Accounts"

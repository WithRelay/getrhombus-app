...
task generate_rmg_data_outbound: :environment do
  ...
  users.each_with_index do |user, i|
    ...
    Message.select(:id, :created_at,
                   :to).where(user_id: user.id).where("`created_at` > '2019-09-03 04:00:00'").find_in_batches(batch_size: 999_998).with_index do |messages, index|
      if messages.present?

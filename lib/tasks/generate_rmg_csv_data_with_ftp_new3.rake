...
task generate_rmg_csv_data_with_ftp_new3: :environment do
  require 'csv'
  require 'tempfile'
  require 'net/sftp'
  ...
  users = User.where(email: ['<redacted_email>'.downcase])
...
=end
  ...
  remote_folder = "/DataGoesHere/Dec 23, 2019 Campaigns Request 1" #{date} Funnel Campaigns - All Accounts"
  ...
  users.each_with_index do |user,i|
    ...
    first_campaign = Campaign.where(user_id: user.id).where("updated_at > '2019-12-01 00:00:00'").order(id: :asc).limit(1).first
    ...
    if first_campaign
      ...
      messages = Message.find_by_sql([query_string, user.id, user.id, '2019-12-01 00:00:00', '2019-12-01 00:00:00', '2019-12-31 00:00:00'])

class Subscription < ActiveRecord::Base
  ...
  def create_subscription(hash)
    ...
    begin
      ...
      uid = team.get_stripe_cred.account_id
  ...
  def cancel_subscription(team, at_period_end = false)
    begin
      res = PaymentService.cancel_subscription(self.stripe_subscription_id, team.get_stripe_cred.account_id, team.is_platform?, at_period_end)
  ...
  def update_subscription(team, coupon_id)
    begin
      res = PaymentService.update_subscription(self.stripe_subscription_id, team.get_stripe_cred.account_id, team.is_platform?, coupon_id)
      if res

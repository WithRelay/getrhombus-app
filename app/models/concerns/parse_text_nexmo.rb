module ParseTextNexmo
  ...
  def process_message(params)  
    begin
      ...
      @msg_text = params[:text].strip
      params[:Body] = params[:text].strip
      params[:From] = params[:msisdn]
      params[:To] = params[:to]
  ...
  def not_repeating_payment?
    ...
    last_messages = Message.where("user_id = ? and created_at >= ?", @this_user.id, Time.current - 5.minutes).order(created_at: :desc)[1..-1]

class Transaction < ActiveRecord::Base
   	...
   	def balanced_debit_customer_card(amount, user, rhombus_number, message)
        ...
        begin
          ...
          if merchant.tax_rate == "0"
            ...
            # save transaction
      		  self.save_transaction(transaction_uri: response.uri, transaction_type: 1, 
                amount: amount_in_hundreds, transaction_number: response.transaction_number, 
                description: response.description, from: user.phone_number,
                to: merchant.rhombus_number, status: response.status, 
                transaction_available_at: response.available_at, last_four: response.source.last_four,
                expiration_month: response.source.expiration_month, 
                expiration_year: response.source.expiration_year, 
                zip_code: response.source.postal_code, card_type: response.source.card_type, 
          			card_name: response.source.name, 
                appear_on_statement_as: response.appears_on_statement_as, tax_rate: merchant.tax_rate,
   ...
   def save_transaction(options = {})
     ...
     #   debugger
      	...
      	self.appear_on_statement_as = options[:appear_on_statement_as] if options[:appear_on_statement_as]

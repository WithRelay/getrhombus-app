...
<redacted_phone_number>,
  ...
  def yo2
    ...
    emails.each do |e|
      u = User.find_by(email: e.downcase)
      if u
        Number.where(user_id: u.id).each_with_index do |n, i|
  ...
  def q
    ary = []
    ary = Number.where(user_id: 51630, provider: 'nexmo')#.order(id: :desc)#.limit(40)
...
<redacted_phone_number>,
  ...
  def wq1
    emails = %w(<redacted_email>)
    rules = Rule.where(user_id: 28681)#.where("response like ?", '%first and last name?%')
    puts "Rules count #{rules.size}"
    rules.each do |r|
      emails.each do |e|
        puts "Selecting account #{e}"

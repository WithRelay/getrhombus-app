class Api::V1::RemindersController < Api::V1::BaseController
  ...
  def create
    begin
      ...
      ExceptionNotifier.notify_exception(e, env: request.env, data: { message: "In v1 RemindersController create", env: Rails.env, reminder: @reminder, list: @list })      
  ...
  def update
    begin
      ...
      ExceptionNotifier.notify_exception(e, env: request.env, data: { message: "In v1 RemindersController update", env: Rails.env, reminder: @reminder })      
      render json: { error: "Something went wrong :(" }, status: 500
    end
  end
  ...
  def reminder_params
    ...
    params.require(:reminder).permit(:channel, :repeat_days, :date_time, :frequency_type, :text).tap do |c|
                                    ...
                                    #c[:date_time] = "2017-07-21 14:35 PM".in_time_zone(current_user.time_zone) 

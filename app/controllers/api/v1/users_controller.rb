    ...
    results = User.find_by_sql [
      "(select customer_id as uid, customer_id_type as uid_type, 
        ...
        (select * from 
          ...
          (select merchant_id, customer_id, 'user' as customer_id_type
          from merchant_customers where merchant_id = ?
          union
          select merchant_id, customer_id, customer_id_type
          from merchant_contacts where merchant_id = ? and customer_id_type = 'user') as A
      ...
      (select customer_id as uid, 'fb_page', name as title, 'Messenger User'
      from merchant_contacts
      inner join fb_creds on fb_creds.page_specific_id = merchant_contacts.customer_id
      where merchant_id = ? and customer_id_type = 'fb_page' and name <> '' and lower(name) like concat('%', ?, '%'))

      union all

      (select customer_id as uid, 'phone_number', 'SMS User', customer_id as description
      from merchant_contacts
      where merchant_id = ? and customer_id_type = 'phone_number' and customer_id like concat('%', ?, '%'))", 

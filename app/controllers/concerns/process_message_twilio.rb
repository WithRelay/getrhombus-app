module ProcessMessageTwilio
  ...
  def process_message(params)  
    ...
    if amount
      if user
        if is_customer_account_complete?(params, user)
          amount = to_cents(amount)
          if is_amount_under_limit?(params, amount)
            if is_merchant_active?(params, merchant)
              if is_merchant_account_complete?(request, params, merchant)
                process_payment(amount, merchant, user, text, params)
              end
            end
          end
        end
      ...
      save_inbound_text(params, msg_code = 5)
    end
  end

  # refactor Transaction model
  def process_payment(amount, merchant, user, text, params)
    # save and pubnub
      new_message = save_inbound_text(params, msg_code = 1)
  ...
  def not_repeating_payment?(id, text)
    last_message = Message.where("user_id_from = ? AND transaction_id > ?", id, 0).order('created_at DESC').limit(1)[0]
  ...
  def is_customer_account_complete?(params, user)
    return true if user.customer_uri
    save_inbound_text(params, msg_code = 7)
    # notify user
    send_response(17, params[:To], params[:From], 
  ...
  def is_merchant_active?(params, merchant)
    return true if merchant.is_active
    save_inbound_text(params, msg_code = 9)

    # notify user
    send_response(19, params[:To], params[:From], "Thank you for sending a payment with Rhombus, but the merchant hasn't completed the account to receive payments.")
  ...
  def is_merchant_account_complete?(params, merchant)
  ...
  def is_amount_under_limit?(params, amount)
    return true if amount <= 1500000
    # send message over limit
    # save in messages and send a response
    save_inbound_text(params, msg_code = 2)
    
    # notify user
    send_response(12, params[:To], params[:From], "Sorry, we are unable to make payments above 15,000 dollars. But you can send in smaller amounts. Thanks!")
  ...
  def save_inbound_text(params, msg_code, transaction_id = 0)           # if not for payment, transaction_id = 0
    #query_hash = Rack::Utils.parse_nested_query(params)                # deal with some weird params from nexmo
    @message = Message.new 
    # SmsMessageSid is now MessageSid
    @message.save_text(from: params[:From], to: params[:To], 
      messageId: params[:MessageSid], text: params[:Body], message_code: msg_code,  
      transaction_id: transaction_id)
      
    # Send to merchant's messaging channel
    RealtimeStreamService.send_message_via_number(params[:From], params[:To], params[:Body], @message.created_at)
    @message
  end

  ...
  def send_message_from_merchant
    if !params[:message].blank?
      ...
      if !user.blank? && !merchant.blank?
        @message = Message.new
        new_message = @message.send_and_save_message(5, merchant.rhombus_number, user.phone_number, params[:message])
        if !new_message.id.blank?
          render :json => Hash['success' => true, 'user_level' => merchant.user_level, 'image_url' => ActionController::Base.helpers.asset_path('rhombus_icon_50x50.png', 'ts_day_of_the_week' => new_message.created_at.strftime('%A'), 'ts_time' => new_message.created_at.strftime('%l:%M %P'))].to_json
          return
        end
      end
    end

class RemindersController < ApplicationController
  ...
  def index
    ...
    if params[:reminders_tomorrow].nil? && params[:reminders_upcoming].nil? && params[:reminders_today].nil?
      ...
      @reminders_tomorrow = params[:reminders_tomorrow].present? ? reminders.where("date_time >= ? AND date_time < ?", Time.current.beginning_of_day + 1.days, Time.current.beginning_of_day + 2.days)
                              .paginate(page: params[:reminders_tomorrow], per_page: PAGINATION_PER_PAGE).order(created_at: :desc) : []

      @reminders_upcoming = params[:reminders_upcoming].present? ? reminders.where("date_time >= ?", Time.current.beginning_of_day + 2.days)
                            .paginate(page: params[:reminders_upcoming], per_page: PAGINATION_PER_PAGE).order(created_at: :desc) : []
      @reminders_today = params[:reminders_today].present? ? reminders.where("date_time >= ? AND date_time < ?",Time.current.beginning_of_day, Time.current.beginning_of_day + 1.days)
                         .paginate(page: params[:reminders_today], per_page: PAGINATION_PER_PAGE).order(created_at: :desc) : []
    end
